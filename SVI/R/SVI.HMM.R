#' Solve the hyperparameters in HMM
#'
#' @param Data_sequence All the observed value, should be a list
#' @param sub_length The length of sub-sequence used in each iteration
#' @param K The number of hidden states
#' @param W_A The initial of the variational parameter for transition matrix
#' @param u The initial of the variational parameter for mean of NIW distribution
#' @param k The initial of the variational parameter for k of NIW distribution
#' @param sigma The initial of the variational parameter for Vaiarance of NIW distribution
#' @param v The initial of the variational parameter for v of NIW distribution.
#' @param U_A The parameter of the prior for transition matrix
#' @param U_phi1 The parameter of the prior for mean of NIW distribution
#' @param U_phi2 The parameter of the prior for k of NIW distribution
#' @param U_phi3 The parameter of the prior for Vaiarance of NIW distribution
#' @param U_phi4 The parameter of the prior for v of NIW distribution
#' @param Pass The number of passes we run, also the maximum number of iteration
#' @param pre Stop Criteria
#' @references Nicholas J. Foti, Jason Xu, Dillon Laird, Emily B. Fox. Stochastic Variational Inference for Hidden Markov Models. \emph{Annual Conference on Neural Information Processing System}, 2014 \cr
#' @author Zhehui Chen, Shiyang Li, Xingguo Li, Tuo Zhao
#' @return All variational parameters in the model.\item{Tran}{The hyperparameter of the transition matrix among hidden states.}\item{mu}{The mean value for each hidden state.}\item{kappa}{The parameter in NIW distribution for each hidden state}\item{Sigma}{The covariance matrix in NIW distribution for each hidden state.}\item{nu}{The parameter in the NIW distribution for each hidden state.}
#' @description In this function, we solve the hidden markov model through stochasitc variational inference. We use the sub-chains to cauculate the variaitional parameters and update them by the formula we get in the reference.
#' @examples ## Generate data in the following way.
#' data <-list(c(1.8,2.0))
#' for(i in 1:10000)
#' {
#'  if(i%%6==0||i%%6==1)
...
#'  }
#' ## run the code
#' result = SVI.HMM(data,sub_length =9999,K=2,W_A, u, k, sigma, v, U_A, U_phi1,U_phi2, U_phi3, U_phi4, Pass=10000, pre=0.01)
