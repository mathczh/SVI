...
)
{
  ...
  W_A1=-1;
  i=0;
  while(norm(W_A1-W_A,"F")>=pre && i<Pass)
  {
    W_A1=W_A;
    i=i+1;
...
alpha=1/K        #
u0 = rchisq(2,1) # initial mean value
...
U_phi4 = v0+2+2  # Hyperperameter the same notation with the paper
sub_length = 5   # sample chain length
SVI.HMM(data,sub_length = 5,K=2,alpha, u0, k0, sigma0, v0, U_A, U_phi1,U_phi2, U_phi3, U_phi4, Pass=1000, 0.001)
